...
foreach ($t in $targets) {
    ...
    if (Test-Path "public/images/gear/$name.*") {
    ...
    Write-Host "Processing $name from $url..."
    try {
        Invoke-WebRequest -Uri $url -UserAgent $ua -OutFile $htmlFile -ErrorAction Stop -TimeoutSec 15
        ...
        if ($imgUrl) {
            ...
            Write-Host "  No image found for $name in $htmlFile"
        }
    }
    catch {
